...
export default function GradeRoutes(app) {
    ...
    const findLatestGradeForQuiz = async (req, res) => {
        const { qid, userId } = req.params;
        try {
            const latestGrade = await dao.findLatestGradeForQuiz(qid, userId);
            if (latestGrade) {
            ...
            console.error("Error finding latest grade:", error);
            res.status(500).json({ error: error.message });
        }
    };

    app.get("/api/courses/:cid/quizzes/:qid/grades/latest/:userId", findLatestGradeForQuiz);
}
